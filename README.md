...
282 existing OSM bridge ways are represented as decks with railings and approximate piers, excluding proposed bridge roads. The offline bridge model reconstructs 1,398 nearby approach ways from the saved map sections, realism tags and recorded road joins; saved map sections omit the original OSM node IDs, so some endpoint connections remain inferred. Heights are estimates: bridge landing elevations use a 6 m offset above the sampled DEM and straight spans ignore the DEM beneath them. Connected approaches taper the *offset* by 0.065 m per metre; terrain slope adds to the resulting road grade. The DEM under bridges may not represent the actual ground or clearance. Car height selection retains deck/ground separation, follows approach ramps, and checks vertical obstacle intervals so water beneath a bridge does not block driving over it. Tunnels are not reconstructed. See [the bridge anomaly report](research/bridges/README.md) for profiles that need inspection.
