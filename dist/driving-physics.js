...
export class CollisionMap{
 ...
 remove(poly){const r=poly.outer,xs=r.map(p=>p[0]),zs=r.map(p=>p[1]);for(let x=Math.floor(Math.min(...xs)/50);x<=Math.floor(Math.max(...xs)/50);x++)for(let z=Math.floor(Math.min(...zs)/50);z<=Math.floor(Math.max(...zs)/50);z++){const k=x+','+z,items=this.cells.get(k);if(!items)continue;const i=items.indexOf(poly);if(i>=0)items.splice(i,1);if(!items.length)this.cells.delete(k);}}
