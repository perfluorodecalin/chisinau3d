import {spatialBatches} from './spatial.js';
...
export async function addStreetDetails(scene,driving,extra={}){
 ...
 function instance(geometry,material,list){for(const part of spatialBatches(list)){const m=new T.InstancedMesh(geometry,material,part.length);part.forEach((o,i)=>{dummy.position.set(o.p[0],o.p[1]+heightAt(o.p[0],o.p[2]),o.p[2]);dummy.rotation.set(0,o.r||0,0);dummy.scale.set(...o.s);dummy.updateMatrix();m.setMatrixAt(i,dummy.matrix);});m.computeBoundingSphere();group.add(m);}}
 ...
 roadSegments.push({a,b,width:roadWidth(t)});const dx=b[0]-a[0],dz=b[1]-a[1],len=Math.hypot(dx,dz);if(!len)continue;const w=roadWidth(t),sw=Math.max(1,Math.min(5,metres(t['sidewalk:width'])||1.8));
 ...
 function nearest(x,z){let n;for(const s of roadSegments){const p=closest(x,z,s.a,s.b);if(!n||p.d<n.d)n={...p,s};}return n;}
 ...
 for(const [positions,color] of [[pavement,'#aaa99b'],[elevatedPavement,'#aaa99b'],[paint,'#f0edda']])if(positions.length){let g=new T.BufferGeometry();g.setAttribute('position',new T.Float32BufferAttribute(positions,3));g=drapeGeometry(g,positions===elevatedPavement?()=>0:heightAt,12);group.add(new T.Mesh(g,new T.MeshStandardMaterial({color,side:T.DoubleSide,roughness:1})));}
