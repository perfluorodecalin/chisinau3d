...
const scene=new T.Scene();const terrain=await loadTerrain(scene);assert.equal(terrain.meta.step,20);assert.match(terrain.meta.source,/Geoportal INDS/);assert.match(terrain.meta.verticalUnit,/inferred/);assert.match(terrain.meta.verticalDatum,/Baltic 1977.*inferred/);assert.ok(scene.children.length>100);assert.equal(scene.children.reduce((n,m)=>n+m.geometry.index.count/3,0),(terrain.meta.nx-1)*(terrain.meta.nz-1)*2);
// The saved snapshot omits node IDs. Coincident motor and pedestrian endpoints
// can belong to different grade-separated networks, so compare each class alone.
const roadClass=h=>/^(motorway|trunk|primary|secondary|tertiary|residential|unclassified|living_street|service|.*_link)$/.test(h)?'motor':'path';
let joinsChecked=0;const heights=new Map();for(const p of Object.values(terrain.profiles))for(const q of [p.points[0],p.points.at(-1)]){const key=q[0]+','+q[1]+','+roadClass(p.highway);if(heights.has(key)){assert.ok(Math.abs(q[2]-heights.get(key))<.01);joinsChecked++;}heights.set(key,q[2]);}
