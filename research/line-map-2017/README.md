...
The runtime **Road geometry A/B** control switches between **Old OSM** and **OSM + LineMap**. The latter draws tinted ribbons along matched LineMap centre lines using the retained OSM road widths, plus thin LineMap edge lines. **View pilot area** moves the atlas camera to the bounded sample. These are visual comparison surfaces over the unchanged OSM roads; driving physics, sidewalks and lamps still use OSM geometry.
